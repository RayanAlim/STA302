...
simulate_data <- function(num_records = 158) {
  ...
  for(year in 2014:2023) {
    for(crime in c("ASSAULT", "AUTOTHEFT", "BIKETHEFT", "BREAKENTER", "HOMICIDE", "ROBBERY", "SHOOTING", "THEFTFROMMV", "THEFTOVER")) {
      data[[paste(crime, year, sep = "_")]] <- sample(0:100, num_records, replace = TRUE)
      data[[paste(crime, "RATE", year, sep = "_")]] <- runif(num_records, 0, 1000)
    }
  }
...
for(year in 2014:2023) {
  for(crime in c("ASSAULT", "AUTOTHEFT", "BIKETHEFT", "BREAKENTER", "HOMICIDE", "ROBBERY", "SHOOTING", "THEFTFROMMV", "THEFTOVER")) {
    required_columns <- c(required_columns, paste(crime, year, sep = "_"), paste(crime, "RATE", year, sep = "_"))
  }
  }
